(function() {
    ...
    app.controller('AllPollsController', AllPollsController);

    function AllPollsController($http, $window, $location, jwtHelper) {
        var vm = this;
        vm.title = "PollsController";
        vm.polls = [];
        vm.poll = {
            ...
            $http.get('/api/user-polls/'+ vm.currentUser.name)
                 .then(function(response) {
                     ...
                     for (var i = 0; i<vm.polls.length; i++){
                         var current = vm.polls[i];
                         var allVotes = 0;
                         for (var j = 0 ; j < current.options.length; j++){
                             allVotes = allVotes + current.options[j].votes;
                         }
                         for (var j = 0 ; j < current.options.length; j++){
                             current.options[j].votes=current.options[j].votes/allVotes*100;
                             current.options[j].votes=current.options[j].votes.toFixed(1)
